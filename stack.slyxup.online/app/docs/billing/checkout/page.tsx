...
// The user must be signed in — the session cookie is sent automatically.
await billing.checkout(planId); // redirects browser to Paddle hosted checkout
...
// [{ id, name, price, interval, features[] }]`}</CodeBlock>
      ...
      <p style={{ color: '#7c8195', fontSize: 14, lineHeight: 1.7 }}>The user must be signed in — the session cookie is sent automatically with the checkout request:</p>
      <CodeBlock>{`await billing.checkout(planId, 'https://example.com/success');
// Browser redirects to Paddle hosted checkout, then back to successUrl`}</CodeBlock>
...
<button disabled={loading} onClick={() => checkout(plans[0].id)}>Upgrade to Pro</button>`}</CodeBlock>

      <h2 style={{ fontSize: 20, fontWeight: 700, marginTop: 32 }}>After payment</h2>
      <p style={{ color: '#9ca3b8', fontSize: 14, lineHeight: 1.7 }}>
        Paddle calls your webhook (<code>POST /v1/webhooks/paddle</code>), the signature is verified, and a subscription row is written to D1. The user is redirected back to your <code>success_url</code>. See <a href="/docs/billing/webhooks" style={{ color: 'var(--accent)' }}>Webhooks</a> for the event lifecycle.
      </p>
    </div>
  );
}
