...
export function PublicNav({ active }: { active?: 'ui' | 'docs' }) {
  const link = (to: string, label: string, isActive?: boolean) => (
    <Link
      ...
      className={`rounded-md px-3 py-2 text-[13px] font-medium transition-colors ${
        isActive ? 'text-white bg-white/10' : 'text-white/55 hover:text-white'
      }`}
    >
      {label}
    </Link>
  ...
  return (
    <header className="sticky top-0 z-30 border-b border-white/[0.08] bg-[#050505]/85 backdrop-blur">
      <div className="mx-auto max-w-[1120px] px-4 sm:px-8 py-3 flex items-center gap-2.5 min-w-0">
        ...
        <nav className="ml-auto flex items-center gap-0.5 shrink-0">
          ...
          <Link
            to="/login"
            className="ml-1.5 rounded-md bg-white text-black px-4 py-2 text-[13px] font-semibold hover:bg-white/85 transition-colors"
          >
            Sign in
          </Link>
        </nav>
...
export function SectionHead({
  ...
  title,
  desc,
}: { eyebrow: string; title: ReactNode; desc?: string }) {
  return (
    <div className="mx-auto max-w-[640px] text-center min-w-0">
      <div className="inline-block rounded-full border border-white/10 bg-white/[0.04] px-3.5 py-1.5 text-[11px] font-semibold uppercase tracking-[0.14em] text-white/55">
        {eyebrow}
      </div>
      <h2 className="font-display mt-4 text-[26px] sm:text-[36px] font-bold leading-[1.05] text-white text-balance">
        {title}
      </h2>
      {desc && (
        <p className="mt-3 text-[13.5px] sm:text-[15px] leading-relaxed text-white/55">
          {desc}
        </p>
      )}
    </div>
        ...
        <div className="flex flex-col md:flex-row gap-8 min-w-0">
          <div className="min-w-0 md:max-w-[300px]">
            ...
            <p className="mt-3 text-[12.5px] leading-relaxed text-white/45">
              Open-source auth & billing on Cloudflare. MIT licensed.
            </p>
          </div>
...
function FooterCol({
  ...
  links,
  external,
}: { title: string; links: Array<[string, string]>; external?: boolean }) {
  return (
